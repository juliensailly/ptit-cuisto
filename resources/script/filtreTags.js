...
function init() {
    document.querySelector("#ingredientSearch").addEventListener("input", function (e) {
        ...
        xhr.open('GET', "index.php?controller=API&action=getTags&searchText=" + document.querySelector("#ingredientSearch").value, true);
        xhr.send();
        xhr.onload = function () {
            if (xhr.status === 200) {
                ...
                for (let i = 0; i < ingredients.length; i++) {
                    ...
                    li.textContent = ingredients[i].tag_title;
                    li.addEventListener("click", function (e) {
                        selectedIngredients[ingredients[i].tag_id] = ingredients[i].tag_title;
...
function updateSelectedIngredients() {
    ...
    for (var tag_id in selectedIngredients) {
        let li = document.createElement("li");
        li.classList.add("list-group-item");
        li.textContent = selectedIngredients[tag_id];
        li.addEventListener("click", function (e) {
            delete selectedIngredients[tag_id];
            updateSelectedIngredients();
            searchForRecipes();
        });
    ...
    if (Object.keys(selectedIngredients).length === 0) {
        document.querySelector('#ingredientsSelected ul').innerHTML = '<div class="alert alert-secondary" role="alert">Aucun tag sélectionné</div>';
    }
}

function searchForRecipes() {
    let url = "http://localhost/index.php?controller=API&action=tagsFilter&tab_ing_id=";
    for (var tag_id in selectedIngredients) {
        url += tag_id + ",";
    }
    if (url[url.length - 1] === ",") {
