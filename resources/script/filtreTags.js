...
function init() {
    ...
    if (
        urlParams.get("tag_id") != null &&
        urlParams.get("tag_id") != "" &&
        urlParams.get("tag_title") != null &&
        urlParams.get("tag_title") != ""
    ) {
        selectedIngredients[urlParams.get("tag_id")] =
            urlParams.get("tag_title");
        window.history.pushState(
            {},
            "",
            "index.php?controller=filtre&action=tagsSearch"
        );
    }
    console.table(selectedIngredients);
    document
        .querySelector("#ingredientSearch")
                ...
                if (xhr.status === 200) {
                    ...
                    for (let i = 0; i < ingredients.length; i++) {
                        ...
                        li.addEventListener("click", function (e) {
                            selectedIngredients[ingredients[i].tag_id] =
                                ingredients[i].tag_title;
                            document.querySelector("#ingredientSearch").value =
                                "";
                            document.querySelector(
                                "#ingredientsResults ul"
                            ).innerHTML = "";
                            updateSelectedIngredients();
                            searchForRecipes();
                        });
                        document
                            .querySelector("#ingredientsResults ul")
                            .appendChild(li);
                    }
                }
            };
